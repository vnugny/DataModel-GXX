...
if __name__ == "__main__":
    ...
    config_json = {
        ...
        "expectations": {
            ...
            # Note: No expectations defined for customer_data
        },
        "notifications": {
    ...
    customer_df = pd.DataFrame({
        ...
        "name": ["Alice Smith", "Bob Johnson", "Carol Williams", "Dave Brown", "Eve Davis"],
        "email": ["alice@example.com", "bob@example.com", "carol@example.com", "dave@example.com", "eve@example.com"],
        "signup_date": ["2022-10-01", "2022-11-15", "2022-12-20", "2023-01-05", "2023-01-10"],
        "is_active": [True, True, False, True, True],
        "loyalty_points": [250, 120, 0, 50, 300],
        "customer_type": ["premium", "standard", "standard", "standard", "premium"]
    })
    
    # Example 1: Basic validation with pre-defined expectations
    print("\n=== Example 1: Basic Validation with Pre-defined Expectations ===")
    ...
    print("Basic data quality check completed!")
    
    # Example 2: Using the Data Assistant to suggest expectations
    print("\n=== Example 2: Using Data Assistant to Suggest Expectations ===")
    
    # Create a temporary DataFrame for demonstration
    suggestions = pipeline.suggest_expectations("customer_data", customer_df)
    print(f"Generated {len(suggestions)} expectations for customer_data")
    
    # Print first few suggestions
    print("\nSample suggested expectations:")
    for i, expectation in enumerate(suggestions[:3]):
        print(f"{i+1}. {expectation['expectation_type']} for {expectation.get('column', 'N/A')}")
    
    # Example 3: Auto-suggesting and validating
    print("\n=== Example 3: Auto-suggesting and Validating ===")
    
    # Create and run the pipeline with auto-suggest enabled
    pipeline = DataQualityPipeline("data_quality_config.json")
    data_map = {
        "sales_data": sales_df,
        "customer_data": customer_df  # No expectations pre-defined for this one
    }
    
    results = pipeline.run_pipeline(
        data_map=data_map,
        output_dir="data_quality_reports",
        auto_suggest=True  # This will auto-generate expectations for customer_data
    )
    
    print("Data quality check with auto-suggested expectations completed!")
    
    # Example 4: Save suggestions to a file for manual review
    print("\n=== Example 4: Save Suggestions to File for Manual Review ===")
    
    expectation_path = pipeline.save_suggested_expectations(
        "product_data", 
        pd.DataFrame({
            "product_id": [1, 2, 3, 4, 5],
            "name": ["Widget A", "Widget B", "Gadget C", "Tool D", "Device E"],
            "price": [9.99, 19.99, 29.99, 39.99, 49.99],
            "category": ["widgets", "widgets", "gadgets", "tools", "devices"],
            "in_stock": [True, True, False, True, True],
            "stock_count": [100, 75, 0, 30, 50]
        }),
        "data_quality_reports/product_expectations.json"
    )
    
    print(f"Saved suggested expectations to {expectation_path}")
    print("Review this file and incorporate into your configuration as needed.")
